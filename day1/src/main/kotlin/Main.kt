fun main(args: Array<String>) {
    ...
    val arr = replaceWords(array, wordToNumber)
    sum = findSum(arr)
    println(sum)
}

fun replaceWords(array: List<String>, pairs: Map<String, String>): Array<String> {
    var arr = emptyArray<String>()
    for (i in array.indices) {
        var s = array[i]
        pairs.forEach {
            s = s.replace(it.key, it.key.first() + it.value + it.key.last(), true)
        }
        arr += s
    }
    return arr
...
fun findSum(array: Array<String>): Int {
    var sum = 0
    val regex = Regex("\\d")
    for (i in array) {
        val matches = regex.findAll(i)
        val numbers = matches.map { result ->
            result.value
        }
        if (numbers.count() > 0) {
...
const val puzzle = """76xkqjzqtwonfour
